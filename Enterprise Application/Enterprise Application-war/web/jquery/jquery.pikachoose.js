...
(function($) {
	...
	$.PikaChoose = function(e, o) {
		...
		if (e instanceof jQuery || e.nodeName.toUpperCase() == 'UL'
			...
			.extend({
				build : function() {
					...
					if (this.options.pikaStage) {
						...
						this.wrap = $("<div class='pika-stage'></div>")
					...
					this.counter = $("<span class='pika-counter'></span>")
							.appendTo(this.imgNav);
					...
					this.caption = $("<div class='caption'></div>")
							.insertAfter(this.imgNav).hide();
					this.tooltip = $("<div class='pika-tooltip'></div>")
							.insertAfter(this.list).hide();
					this.aniWrap = $("<div class='pika-aniwrap'></div>")
							.insertAfter(this.caption);
					this.aniImg = $("<img>").appendTo(this.aniWrap).hide();
					this.aniDiv = $("<div class='pika-ani'></div>").appendTo(
							this.aniWrap);
					this.textNav = $("<div class='pika-textnav'></div>")
							.insertAfter(this.aniWrap);
					this.textPrev = $(
					...
					this.loader = $("<div class='pika-loader'></div>")
							.appendTo(this.wrap).hide().html(
									this.options.text.loading);
					...
					this.finishAnimating({
						...
						'clickThrough' : this.active.parent().attr('href')
								|| "",
						'clickThroughTarget' : this.active.parent().attr(
								'target')
								|| "",
					...
					if (this.options.carousel) {
										...
										.click(
												function(e, x) {
													...
													var last = (jQuery(this)
															...
															.find('li:last')
															.attr(
																	'jcarouselindex') == clicked - 1) ? true
															: false;
													if (!last) {
														clicked = (clicked - 2 <= 0) ? 0
																: clicked - 2
													}
													clicked++;
						...
						var carouselOptions = $.extend({}, carouselDefaults,
				...
				createThumb : function(ele) {
					...
					$.data(ele[0], 'clickThrough', self.parent('a')
							.attr('href')
							|| "");
					$.data(ele[0], 'clickThroughTarget', self.parent('a').attr(
							'target')
							|| "");
					...
					$('<img />')
							.bind(
									...
									function() {
										...
										var ph = parseInt(self.parents('.clip')
												.css('height').slice(0, -2));
										var pw = parseInt(self.parents('.clip')
												.css('width').slice(0, -2));
										if (pw == 0) {
											pw = self.parents('li:first')
													.outerWidth()
										}
										if (ph == 0) {
											ph = self.parents('li:first')
													.outerHeight()
										}
										var rw = pw / w;
										var rh = ph / h;
				...
				bindEvents : function() {
					...
					this.imgPlay.unbind('click.pikachoose').bind(
							'click.pikachoose', {
								self : this
							}, this.playClick);
					this.wrap.unbind('mouseenter.pikachoose').bind(
							'mouseenter.pikachoose',
							{
								self : this
							},
							function(e) {
								e.data.self.imgNav.stop(true, true).fadeTo(
										'slow', 1.0);
								if (e.data.self.options.hoverPause == true) {
									clearTimeout(e.data.self.timeOut)
								}
							});
					this.wrap.unbind('mouseleave.pikachoose').bind(
							'mouseleave.pikachoose',
							{
								self : this
							},
							function(e) {
								e.data.self.imgNav.stop(true, true).fadeTo(
										'slow', 0.0);
								if (e.data.self.options.autoPlay == true
										&& e.data.self.options.hoverPause) {
									e.data.self.timeOut = setTimeout((function(
											self) {
										return function() {
											self.nextClick()
										}
					...
					this.tooltip.unbind('mouseenter.pikachoose').bind(
							'mouseenter.pikachoose', {
								self : this
							}, function(e) {
								clearTimeout(e.data.self.stillOut)
							});
					this.tooltip.unbind('mouseleave.pikachoose').bind(
							'mouseleave.pikachoose',
							{
								self : this
							},
				...
				imgClick : function(e, x) {
					...
					self.animating = true;
					self.active.fadeTo(300, self.options.thumbOpacity)
							.removeClass('active');
				...
				doAnimation : function(n, data) {
					...
					if (img.height != self.image.height()
					...
					case 1:
						self.aniDiv.hide();
						self.aniImg.height(self.image.height()).hide().attr(
								'src', data.source);
						self.wrap.css({
							height : self.image.height()
						});
						...
						break;
					case 2:
						self.aniDiv.show().children().hide().each(
								function(index) {
									var delay = index * 30;
									$(this).css({
										opacity : 0.1
						...
						break;
					case 3:
						self.aniDiv.show().children("div:lt(5)").hide().each(
								function(index) {
